...
opponents = [
    ["reallySC2Bot", "P", "python"],
    ["whatSC2Bot", "Z", "python"],
    ["whySC2Bot", "T", "python"],
]
map_list = ["IncorporealAIE_v4", "AcropolisAIE", "LeyLinesAIE_v3", "MagannathaAIE_v2", "PersephoneAIE_v4", "PylonAIE_v4", "TorchesAIE_v4", "UltraloveAIE_v4"]
...
'''def get_matches_to_play():
    """
    Returns a list of matches to play.
    Each opponent plays on each map.
    """
    matches = []
    runner_id = 1
    ...
    for opponent in opponents:
        for map_name in map_list:
            matches.append((runner_id, bot, opponent, map_name))
            runner_id += 1
...
def play_game(match):
    try:
        ...
        command = f'docker compose -p {runner_id} -f {docker_compose_file} up --abort-on-container-exit'
        subprocess.Popen(
            command,
            shell=True,
        ).communicate()
